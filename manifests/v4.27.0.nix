{
  ...
  # Custom buildLeanPackage with lndir-based depRoot to fix module public imports
  # The issue: Lean 4.27.0's new `module` keyword with `public import` creates transitive
  # dependencies that require all dependent .olean files available during module initialization.
  # The old `cp -drsu` approach creates symlinks to directories, which don't properly merge
  # when multiple depRoots have overlapping directory structures.
  # The fix: Use `lndir` which creates real directories with symlinked leaf files,
  # allowing proper merging of multiple dependency trees.
  buildLeanPackage = {
    lean,
    lean-leanDeps ? lean,
    lean-final ? lean,
    ...
    lib.makeOverridable (
      {
        ...
        with builtins; let
          ...
          copyToStoreSafe = base: suffix:
            if lib.isDerivation base
            then base + suffix
            else
              builtins.path {
                name = lib.strings.sanitizeDerivationName (baseNameOf suffix);
                path = base + suffix;
              };
          modToLean = mod: copyToStoreSafe src "/${modToPath mod}.lean";
          bareStdenv = ./bareStdenv;
          mkBareDerivation = args:
            derivation (args
              // {
          ...
          runBareCommandLocal = name: args: buildCommand:
            runBareCommand name (args
              // {
          ...
          mkSharedLib = name: args:
            ...
            '';
          # Use lndir instead of cp -drsu to properly merge directory trees.
          # lndir creates real directories with symlinked files, allowing proper
          # merging when multiple sources have overlapping directory structures.
          depRoot = name: deps:
            mkBareDerivation {
              name = "${name}-depRoot";
              inherit deps;
          ...
          submodules = mod: let
            ...
            f = p: t:
              if t == "directory"
              then submodules "${mod}.${p}"
              else let
                m = builtins.match "(.*)\.lean" p;
              in
                lib.optional (m != null) "${mod}.${head m}";
          in
            concatLists (lib.mapAttrsToList f dir);
          ...
          expandGlobAllApprox = g:
            ...
            then
              ...
              let
                root = head (split "\\." g);
              in
                lib.optional (pathExists (src + "/${modToPath root}.lean")) root ++ lib.optionals (pathExists (modToAbsPath root)) (submodules root)
            else if g.glob == "one"
            then expandGlobAllApprox g.mod
            else if g.glob == "submodules"
            then submodules g.mod
            else if g.glob == "andSubmodules"
            then [g.mod] ++ submodules g.mod
          ...
          compileMod = mod: drv:
            mkBareDerivation {
              ...
              buildCommand = ''
                ...
                leanc -c -o $out/$oPath $leancFlags -fPIC ${
                  if debug
                  then "${drv.c}/${drv.cPath} -g"
                  else "src.c -O3 -DNDEBUG -DLEAN_EXPORTING"
                }
              '';
            };
          mkMod = mod: deps: let
            drv = buildMod mod deps;
          ...
          modCandidates = mapAttrs (mod: header: let
            deps =
              if header.errors == []
          ...
          in
            mkMod mod (map (dep:
              if modDepsMap ? ${dep}
              then modCandidates.${dep}
              else externalModMap.${dep})
            deps))
          modDepsMap;
          expandGlob = g:
            if typeOf g == "string"
          ...
          mods' = listToAttrs (map (e: {
              name = e.key;
              value = modCandidates.${e.key};
            }) (genericClosure {
              startSet = map (m: {key = m;}) (concatMap expandGlob roots);
              operator = e:
                if modDepsMap ? ${e.key}
                then map (m: {key = m.module;}) (filter (m: modCandidates ? ${m.module}) modDepsMap.${e.key}.result.imports)
                else [];
            }));
        ...
        in rec {
          ...
          cTree = symlinkJoin {
            name = "${name}-cTree";
            paths = map (mod: mod.c) (attrValues mods);
          };
          oTree = symlinkJoin {
            name = "${name}-oTree";
            paths = attrValues objects;
          };
          iTree = symlinkJoin {
            name = "${name}-iTree";
            paths = map (mod: mod.ilean) (attrValues mods);
          };
          ...
          in
            runCommand executableName {buildInputs = [stdenv.cc leanc];} ''
              mkdir -p $out/bin
