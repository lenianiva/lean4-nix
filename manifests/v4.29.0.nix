{
  ...
  bootstrap = {
    ...
    mimalloc-patch = pkgs.writeText "mimalloc.patch" ''
      --- a/CMakeLists.txt
      +++ b/CMakeLists.txt
      @@ -80,11 +80,7 @@ if(USE_MIMALLOC)
         ExternalProject_Add(
           mimalloc
           PREFIX mimalloc
      ...
      -    CONFIGURE_COMMAND ""
      -    BUILD_COMMAND ""
      +    SOURCE_DIR "/nix/store/9ljhaakj0s6fdrzhcr5r9030d9w83gg9-source"
           INSTALL_COMMAND ""
         )
    ...
    with builtins; rec {
          ...
          // {
            ...
            cmakeFlags =
              ...
              ++ (
                args.cmakeFlags or [
                  "-DSTAGE=1"
                  "-DPREV_STAGE=./faux-prev-stage"
                  "-DUSE_GITHASH=OFF"
                  "-DCADICAL=${cadical}/bin/cadical"
                ]
              )
              ++ (args.extraCMakeFlags or extraCMakeFlags)
              ++ lib.optional (args.debug or debug) ["-DCMAKE_BUILD_TYPE=Debug"];
            preConfigure =
      ...
      in
        (all: all // all.lean) rec {
          ...
          Std =
            attachSharedLib leanshared Std'
            // {
              allExternalDeps = [Init];
            };
          Lean =
            attachSharedLib leanshared Lean'
            // {
              allExternalDeps = [Std];
            };
          Lake = build {
          ...
          stdlibLinkFlags = "${
            lib.concatMapStringsSep " " (l: "-L${l.staticLib}") stdlib
          } -L${leancpp}/lib/lean";
          libInit_shared =
          ...
          leanshared_1 =
            ...
            '';
          leanshared_2 =
            runCommand "leanshared_2"
            {
              buildInputs = [stdenv.cc];
              libName = "leanshared_2${stdenv.hostPlatform.extensions.sharedLibrary}";
            }
            ''
              mkdir $out
              touch empty.c
          ...
          lean-all = stdenv.mkDerivation {
            ...
            buildCommand = ''
              mkdir -p $out/bin $out/lib/lean
              ln -sf ${leancpp}/lib/lean/* ${
                lib.concatMapStringsSep " " (l: "${l.modRoot}/* ${l.staticLib}/*") (lib.reverseList stdlib)
              } ${libInit_shared}/* ${leanshared}/* ${leanshared_1}/* ${leanshared_2}/* ${Lake.sharedLib}/* $out/lib/lean/
